...
const makeTokenRevoker =
  ({ client_id, client_secret, errorHandler }) =>
  ({ revocation_endpoint, token_hint, token }) =>
...
logout.forceLogoutAndRevoke = (context) => {
  ...
  return async (req, res /* , next */) => {
    const { tokens } = req.userContext || {};
    const { end_session_endpoint, revocation_endpoint } = req.oidc_metadata;
    ...
    const endOktaSessionEndpoint = `${end_session_endpoint}?${querystring.stringify(
      params,
    )}`;
